...
public class FsNotification extends BroadcastReceiver {
    ...
    private void setupNotification(Context context) {
        ...
        int icon = R.mipmap.launcher/*notification*/;
        CharSequence tickerText = String.format(context.getString(R.string.notif_server_starting), iptext);
        long when = System.currentTimeMillis();

        // Define Notification's message and Intent
        CharSequence contentTitle = context.getString(R.string.ptx_notif_title);
        CharSequence contentText = String.format(context.getString(R.string.notif_text), iptext);

        Intent notificationIntent = new Intent(context, PTMainActivity.class);/*FsPreferenceActivity.class*/
        notificationIntent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_SINGLE_TOP);
        PendingIntent contentIntent = PendingIntent.getActivity(context, 0, notificationIntent, 0);

        int stopIcon = android.R.drawable.ic_menu_close_clear_cancel;
        CharSequence stopText = context.getString(R.string.ptx_notif_stop_text);
        ...
        Notification.Builder nb = new Notification.Builder(context)
                ...
                .setColor(context.getResources().getColor(R.color.ptx_button_background_blue))
        ...
        if (VERSION.SDK_INT >= VERSION_CODES.JELLY_BEAN) {
            ...
            //去掉设置按钮
            //nb.addAction(preferenceIcon, preferenceText, preferencePendingIntent);//Victor modify
            nb.setShowWhen(true);
        ...
        //nm.notify(NOTIFICATIONID+1, notification);

        RemoteViews mRemoteViews = new RemoteViews(context.getPackageName(), R.layout.layout_notification);
        mRemoteViews.setOnClickPendingIntent(R.id.icon, contentIntent);
        mRemoteViews.setOnClickPendingIntent(R.id.content, contentIntent);
        mRemoteViews.setOnClickPendingIntent(R.id.tvStop, stopPendingIntent);
        //mRemoteViews.setOnClickPendingIntent(R.id.tvStop, stopPendingIntent);
        String format = new SimpleDateFormat("HH:mm").format(new Date());
        mRemoteViews.setTextViewText(R.id.title, contentTitle.toString());
        mRemoteViews.setTextViewText(R.id.subtitle,contentText.toString());
        mRemoteViews.setTextViewText(R.id.time, format);

        //NotificationCompat.Builder nb= new NotificationCompat.Builder(context);
        notification = new Notification(R.mipmap.launcher, "", System.currentTimeMillis());
        notification.contentIntent = null;
        notification.flags = Notification.FLAG_NO_CLEAR;
        notification.priority = Notification.PRIORITY_MAX;
        notification.contentView = mRemoteViews;
        //notification.actions = new Notification.Action[]{new Notification.Action(stopIcon, stopText, stopPendingIntent)};
